...
public:
    ...
    int getNumGlobalMinima() const { return option.GKLS_glob.num_global_minima; }

    vector<double> getGlobalMinimumPoint(int idx) const {
        vector<double> point(mDimension);
        if (idx < (int)option.GKLS_glob.num_global_minima) {
            double* x = option.GKLS_minima.local_min[option.GKLS_glob.gm_index[idx]];
    ...
    double getGlobalMinimumValue() const { return option.GKLS_global_value; }

    bool isGlobalMinimumPoint(const vector<double>& point, double tolerance = 1e-6) const {
        double val = Compute(0, point);
        if (fabs(val - option.GKLS_global_value) > tolerance) return false;

        for (unsigned int i = 0; i < option.GKLS_glob.num_global_minima; i++) {
            double dist = option.GKLS_norm(point.data(),
                option.GKLS_minima.local_min[option.GKLS_glob.gm_index[i]],
                mDimension);
